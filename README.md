...
An `environment.yml` is included as a fallback. Use it only if the standard Anaconda environment does not work:
...
Parquet is the default because the dataset has 9,986,098 rows and loads roughly ten times faster in that format.